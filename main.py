...
def fit(genomes, config):
    ...
    for _, g in genomes:
        ...
        nets.append(net)
        birds.append(Bird(WIN_WIDTH/2 - 100, WIN_HEIGHT/2 - 100))
        g.fitness = 0
        ge.append(g)
    ...
    while run:
        ...
        for x, bird in enumerate(birds):
            ...
            output = nets[x].activate((bird.y, bird.x, abs(bird.y - pipes[pipe_ind].height), abs(bird.y - pipes[pipe_ind].bottom)))
        ...
        for pipe in pipes:
            for x, bird in enumerate(birds):
                ...
                if not pipe.passed and pipe.x + 20 < bird.x:
                    pipe.passed = True
                    add_pipe = True
